...
async function main() {
  ...
  console.log("Saving jump test crops...");
  for (let i = 0; i < jumpCols.length; i++) {
    const [xMin, xMax] = jumpCols[i];
    const cropped = image.clone().crop({ x: xMin, y: 20, w: xMax - xMin, h: 115 });
    floodFillBackground(cropped);
    const finalImg = autoCrop(cropped);
    await finalImg.write(path.join(outputDir, `test_jump_${i + 1}.png`));
  }
  console.log("Jump splits completed!");
}
